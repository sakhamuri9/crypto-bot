"""
Script to visualize multiple support and resistance levels for BTC-USDT and SUI-USDT.
"""
import os
import logging
import pandas as pd
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('sr_visualization.log')
    ]
)
...
def get_data(symbol, timeframe='hours', interval='1h', limit=2000):
    ...
    Args:
        ...
        interval (str): Interval for the data (e.g., '1h', '1d')
        limit (int): Maximum number of records to return
        
    Returns:
        pandas.DataFrame: DataFrame with OHLCV data and indicators
    """
    logger.info(f"Collecting data from CoinDesk for {symbol} with {timeframe} timeframe")
...
def detect_multiple_sr_levels(df, symbol, num_levels=5):
    ...
    """
    logger.info(f"Detecting multiple S/R levels for {symbol}")
    
    parameter_sets = [
        (7, 40, 5, num_levels, 1, 0.8),  # High resistance bias
        (10, 30, 7, num_levels, 2, 0.7),  # Medium resistance bias
        (15, 20, 10, num_levels, 3, 0.6),  # Low resistance bias
        (5, 50, 3, num_levels, 1, 0.9),   # Very high resistance bias
        (12, 25, 8, num_levels, 2, 0.65)   # Custom balance
    ]
...
def visualize_sr_levels(df, symbol, resistance_levels, support_levels):
    ...
    """
    logger.info(f"Visualizing multiple S/R levels for {symbol}")
    
    plt.figure(figsize=(14, 7))
    
    plt.plot(df.index[-200:], df['close'].iloc[-200:], label='Close Price', color='blue', alpha=0.7)
    ...
    plt.title(f'{symbol_name} Price with Multiple Support and Resistance Levels')
    plt.xlabel('Date')
    plt.ylabel('Price')
    plt.legend(loc='best')
    ...
    save_path = f"results/multiple_sr_levels_{symbol.replace('-', '_')}_{timestamp}.png"
...
def analyze_sr_levels(symbol, resistance_levels, support_levels, current_price):
    ...
    analysis = f"\n{'='*50}\n"
    analysis += f"SUPPORT AND RESISTANCE ANALYSIS FOR {symbol_name}\n"
    analysis += f"{'='*50}\n\n"
...
def main():
    """
    Main function to visualize multiple support and resistance levels.
    """
    logger.info("Starting multiple S/R level visualization")
    
    os.makedirs('results', exist_ok=True)
    
    symbols = [
        ('BTC-USDT-VANILLA-PERPETUAL', 'hours', '1h'),
        ('SUI-USDT-VANILLA-PERPETUAL', 'days', '1d')
    ]
    ...
    logger.info("Multiple S/R level visualization completed")
